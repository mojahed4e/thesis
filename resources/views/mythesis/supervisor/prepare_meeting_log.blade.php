...
if($meetinglogifo[0]->supervisor_approval_status == 0)
	$pagetitle = 'Approve Meeting Minutes - '.$logseq.' Schedule';
else
	$pagetitle = 'Complete Meeting Minutes - '.$logseq;
@endphp
		...
		<div class="row">
			<div class="col-md-12">
				...
				@method('post')
                    ...
                    $aThesisTimeline = \App\ThesisProgressTimeline::select('thesis_progress_timeline.*')
                    							->where(['thesis_progress_timeline.item_id' => $iteminfo[0]->id])->get();
	                  $aTimelineSeqArray =json_decode(json_encode($aThesisTimeline), true);
	                  $aPrevMeetingInfo = array();
                    if(count($progresschecklist) > 0){
                    	...
                    	if($progresschecklist[0]->checklist_type == 1){
                    		...
                    		if($logseq > 1){
                    			$aPrevMeetingInfo =\App\MeetingLogs::select("meeting_logs.*")->where(["meeting_logs.item_id" => $iteminfo[0]->id, "meeting_logs.meeting_log_seq" => ($logseq-1),'meeting_logs.meeting_log_type' => $progresschecklist[0]->checklist_type])->get();
                    		}
                    	}
                    	else{
                    	...
                    	$vSelDate = \Carbon\Carbon::parse($meetinglogifo[0]->meeting_date)->format('d-m-Y');
                    }
                    @endphp
	                    ...
	                    @endif
						          ...
						          <td class="cbold_text">{{ __('Date of previous meeting') }}</td>
											<td class="sid_text">
											@if(count($aPrevMeetingInfo) > 0)
												@if(!empty($aPrevMeetingInfo[0]->next_meeting_date))
                      		{{\Carbon\Carbon::parse($aPrevMeetingInfo[0]->meeting_date)->format('d-m-Y')}}
                      	@else
                      		{{ __('N/A') }}
                      	@endif
	                    @else
                  		...
                  		@endif
										...
										@if($meetinglogifo[0]->supervisor_approval_status == 1 && (($meetinglogifo[0]->meeting_log_type == 1 && $logseq <= 5) || ($meetinglogifo[0]->meeting_log_type == 2 && $logseq < 5)))
										<tr>
											<td class="cbold_text"><span class="mark">*</span>{{ __('Next meeting date') }}ss</td>
											<td class="cbold_text">
												<input type="text"  name="nextmeetingdate" id="nextmeetingdate" placeholder="{{ __('Select date') }}" class="form-control" value="{{ ($meetinglogifo[0]->next_meeting_date ? \Carbon\Carbon::parse($meetinglogifo[0]->next_meeting_date)->format('m/d/Y') : '') }}"/> 
											</td>
											<td colspan="2">&nbsp;</td>
										</tr>
										@endif										
			                    ...
			                    @endif
				                  ...
				                  <label class="col-form-label cbold_text pb-3" style="padding:0px;">{{ __('Last meeting agenda:') }}</label>
				                    ...
				                    {!! $previousmeetinglogifo[0]->next_meeting_agenda !!}
											...
											@endif	
										@endif
										@if($progresschecklist[0]->student_upload_status >= 2 && (($meetinglogifo[0]->meeting_log_type == 1 && $logseq <= 5) || ($meetinglogifo[0]->meeting_log_type == 2 && $logseq < 5)))
										<tr style="background-color:#e5e5e5;">
											<td style="text-align: center;" colspan="4">
			                  ...
			                  <label class="col-form-label cbold_text" style="padding:0px;">{{ __('Next meeting agenda:') }}</label>
			                    <span class="form-group view_word  {{ $errors->has('nextmeetingagenda') ? ' has-danger' : '' }}">
			                    <textarea name="nextmeetingagenda" id="input-nextmeetingagenda" cols="35" rows="15" class="form-control{{ $errors->has('nextmeetingagenda') ? ' is-invalid' : '' }}" placeholder="{{ __('Next meeting agenta') }}" value="{{ old('nextmeetingagenda','') }}" readonly>{{ old('nextmeetingagenda', '') }}</textarea>
		                      @include('alerts.feedback', ['field' => 'nextmeetingagenda'])</span>
            						</td>
										</tr>		
										@endif		
							...
							<div class="card-body" style="padding:0px;">
								<dir class="row">									
									<div class="col-md-11 text-center" style="padding:0px;">											
										@if($progresschecklist[0]->student_upload_status == 2)
											<button type="button" id="postprogresscomment" onclick="funLogSubmmitValidate(2)" class="btn bt_styl btn_txtbold">{{ __('Complete Meeting Minutes') }}</button>
											<button type="button" id="postprogresscomment" onclick="funCancelLog()" class="btn bt_styl btn_txtbold">{{ __('Cancel') }}</button>
										@elseif($progresschecklist[0]->student_upload_status == 1)
											<button type="button" id="postprogresscomment" onclick="funLogSubmmitValidate(1)" class="btn bt_styl btn_txtbold">{{ __('Approve Schedule') }}</button>
											<button type="button" id="postprogresscomment" onclick="funCancelLog()" class="btn bt_styl btn_txtbold">{{ __('Cancel') }}</button>											
										@else
											<button type="button" id="postprogresscomment" onclick="funCancelLog()" class="btn bt_styl btn_txtbold">{{ __('Back to Details') }}</button>
										@endif
...
<script>
  ...
  $(document).ready(function() {
  	...
  	var vAction = "{{request()->get('action')}}";	  	  	
  	if(Date.parse(startDate) > Date.parse(endDate)){ 
  		swal({
			  title: 'Completion Date Missed!',
			  text: 'Kindly note student had missed the scheduled meeting completion date. Please contact program manager immediately to modify the completion date.',
			  type: 'warning',
			  confirmButtonColor: '#ea2c6d'
			}).then ((result) =>{
				window.location.href = '{{ url("/mythesis/detail?") }}'+{{$iteminfo[0]->id}}+'&tab=4&action='+vAction;
			})
  	}	  	
  	var vDate = new Date();    	
...
function funCancelLog() {
	...
	if(vRoleId == 2)
		window.location.href = '{{ url("/mythesis/detail?") }}'+{{$iteminfo[0]->id}}+'&tab=4&action=ac';	
	else
