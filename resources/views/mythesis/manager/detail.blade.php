					...
					<div class="card-body">
						...
						<div class="tab-content tab-space">
							<div class="tab-pane active" id="link1">
								<dir class="row">
									<div class="col-md-6">
										<table class="table">    
											...
											<tr>
												<td class="cbold_text">{{ __('Keyword') }}</td>
												<td class="sid_text">
													@php
												...
												<tr>
													...
													@endif									
														...
														@foreach ($supervisors as $supervisor)
															@if(count($prefsupervisor))
																@if($supervisor->id == $prefsupervisor[0]->id)
																	{{$supervisor->name}}
																@endif
															@elseif($item->assigned_to > 0)
																@if($supervisor->id == $item->assigned_to) 
																		{{$supervisor->name}}
																@endif
															@else
																@if($PrefNull == 0)
								...
								@if(auth()->user()->role_id == 2 && $item->approval_status == 1 && request()->get('action') != "ac" )									
									<div class="row">								  
									  ...
									  <div class="col-sm-7">
										<div class="form-group view_word {{ $errors->has('supervisor_id') ? ' has-danger' : '' }}">
											...
											<option value="">--</option>
											@foreach ($supervisors as $supervisor)
												@if($supervisor->availabe_flage == 1 || ($supervisor->id == $item->assigned_to && $supervisor->availabe_flage == 2))		
													@if(count($prefsupervisor))
														@if($supervisor->id == $prefsupervisor[0]->id) <option value="{{ $supervisor->id }}" selected> {{$supervisor->name}}</option>
														@else
													...
													@else
														@php
														$vCProgams = explode(',',$supervisor->program_availability);
														@endphp
														@if(in_array($item->program_id,$vCProgams))
														<option value="{{ $supervisor->id }}"> {{$supervisor->name}}</option>
														@endif
													@endif
												@endif
									...
									<div class="row">								  
									  ...
									  <div class="col-sm-7">
										<div class="form-group view_word {{ $errors->has('cosupervisor_id') ? ' has-danger' : '' }}">
											...
											@foreach ($supervisors as $supervisor)
												@php
												if(!empty($supervisor->program_availability)){
													$vCProgams = explode(',',$supervisor->program_availability);
												}
												else
													$vCProgams = array();
												@endphp
												@if(count($prefsupervisor) > 0)
													@if($supervisor->id != $prefsupervisor[0]->id && $supervisor->id != $item->assigned_to && in_array($item->program_id,$vCProgams))
														<option value="{{ $supervisor->id }}" {{ $supervisor->id == $item->cosupervisor ? 'selected' : '' }}>{{ $supervisor->name }}</option>
													@endif
												@else
													@if($supervisor->id != $item->assigned_to && in_array($item->program_id,$vCProgams))
														<option value="{{ $supervisor->id }}" {{ $supervisor->id == $item->cosupervisor ? 'selected' : '' }}>{{ $supervisor->name }}</option>
													@endif
												@endif
											@endforeach
									...
									<div class="row">								  
									  <label class="col-sm-3 col-form-label cht_text"><span class="mark">*</span>{{ __('Assign Examine') }}
									  </label>
									  @php
									  $aPanelMembers = array();
									  if(count($panelmembers) > 0){
									  	foreach ($panelmembers as $paelmember){
									  		$aPanelMembers[] = $paelmember->id;
									  	}
									  } 									  
									  @endphp
									  <div class="col-sm-7">
										<div class="form-group view_word {{ $errors->has('thesis_examine') ? ' has-danger' : '' }}">
											<select class="selectpicker col-sm-9 pl-0 pr-0" name="thesis_examine[]" id="thesis_examine" data-style="select-with-transition" onchange="funLimitExamineSelection()" multiple title="-" data-size="10" data-placeholder="Select multiple options" data-live-search="true">
											<option value="">-</option>
											@foreach ($supervisors as $supervisor)
												@php
												if(!empty($supervisor->program_availability)){
												...
												@endphp
												@if(count($prefsupervisor) > 0)
													@if($supervisor->id != $prefsupervisor[0]->id && $supervisor->id != $item->assigned_to && $supervisor->id != $item->cosupervisor && in_array($item->program_id,$vCProgams))
														<option value="{{ $supervisor->id }}" {{ (count($aPanelMembers) > 0) ? (in_array($supervisor->id, $aPanelMembers) ?  'selected' : '') : '' }}>{{ $supervisor->name }}</option>
													@endif
												@else
													@if($supervisor->id != $item->assigned_to && $supervisor->id != $item->cosupervisor && in_array($item->program_id,$vCProgams))
														<option value="{{ $supervisor->id }}" {{ (count($aPanelMembers) > 0) ? (in_array($supervisor->id, $aPanelMembers) ?  'selected' : '') : '' }}>{{ $supervisor->name }}</option>
													@endif
												@endif
											@endforeach
									...
									<div class="row">
									  ...
									  <div class="col-sm-7">
										<div class="form-group view_word {{ $errors->has('approve_status') ? ' has-danger' : '' }}">
											<select class="selectpicker col-sm-5 pl-0 pr-0" name="approve_status" id="approve_status" onchange="funUpdateStatus()" data-style="select-with-transition" title="" data-size="100" disabled="disabled">
											<option value="">-</option>
									...
									@php
										if($requestdetails[0]->manager_approval_status == 0  || request()->get('action') != 'ac') 
									...
									@if(($item->approval_status == 1 && $item->assigned_to	!= auth()->user()->id) || ($item->approval_status == 1 && $item->assigned_to	== auth()->user()->id && $requestdetails[0]->supervisor_acceptence_status == 1 ))
									<div class="row" style="text-align:center;">
										...
										<div class="col-sm-7">
											...
											@if(auth()->user()->manager_flag != 2 && $requestdetails[0]->progress_completion != 3)
												<button type="button" onclick="funUpdateTheisiDetails()" class="btn bt_styl btn_txtbold">{{ __('Save') }}</button>
											@endif														
										</div>
									</div>
									@endif
...
<script>
  ...
  $(document).ready(function() {
	...
	function funUpdateTheisiDetails() {		
	...
	function funPrepareMeetingMinutes(pmTerm,pmSeq) {
		...
		document.frmCommentUpdate.action='{{ url("/mythesis/") }}/'+{{$item->id}}+'/prepare-meeting-minutes?tab=4';
		document.frmCommentUpdate.method='POST'
		document.frmCommentUpdate.submit();		
	}
	...
	function funViewMeetingMinutes(pmTerm,pmSeq) {			
		...
		$("#term_seq").val(pmTerm);
		$("#viewflag").val(1);		
		document.frmCommentUpdate.action='{{ url("/mythesis/") }}/'+{{$item->id}}+'/view-meeting-minutes?tab=4';
		document.frmCommentUpdate.method='POST'
		document.frmCommentUpdate.submit();		
	}
	function funViewRubric(pmTerm,pmType,pmCreatedBy) {    	
    	$("#rubricterm").val(pmTerm);
    	$("#rubrictype").val(pmType);
    	$("#rubriccreatedby").val(pmCreatedBy);
	...
	function funLimitExamineSelection(){
		var supervisor = $("#term_seq").val(pmTerm);
		var vcosupervisor = $("#term_seq").val(pmTerm);
		var selExamine = $('#thesis_examine').val(); 
		if(selExamine.length > 2){
