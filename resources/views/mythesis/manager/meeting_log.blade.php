...
if($meetinglogifo[0]->supervisor_approval_status == 0)
	...
	$pagetitle = 'Complete Meeting Minutes ';
@endphp
	                    ...
	                    @endif
						          ...
						          <td class="cbold_text">{{ __('Date of previous meeting') }}</td>
											...
											@if(count($meetinglogifo) > 0)
												@if($meetinglogifo[0]->supervisor_approval_status == 1 && !empty($meetinglogifo[0]->next_meeting_date))
                      		{{\Carbon\Carbon::parse($meetinglogifo[0]->next_meeting_date)->format('d-m-Y')}}
                      	@elseif(!empty($meetinglogifo[0]) && !empty($meetinglogifo[0]->next_meeting_date))
                      		{{\Carbon\Carbon::parse($meetinglogifo[0]->next_meeting_date)->format('d-m-Y')}}
                      	@else
                      		{{ __('N/A') }}
                      	@endif
