...
<link href="{{ asset('material') }}/css/student-thesis.css" rel="stylesheet" >
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
		  <form method="post" name="frmRequestUpdate" id="frmRequestUpdate" enctype="multipart/form-data" action="{{ route('item.update-student-thesis', $item) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')            
            <div class="card ">
              <div class="card-header">                
                <h4 class="card-title view_word">{{ __('Edit Thesis') }}</h4>
              </div>
              <div class="card-body ">               
                <div class="row">
                  <label class="col-sm-3 col-form-label form_chg">{{ __('Title') }}</label>
                  <div class="col-sm-7">
                ...
                <div class="row">
                  <label class="col-sm-3 col-form-label form_chg">{{ __('Prefered Supervisor') }}</label>
                  <div class="col-sm-7">
                    <div class="form-group view_word {{ $errors->has('supervisor_id') ? ' has-danger' : '' }}">
                        ...
                        @foreach ($supervisors as $supervisor)
                          ...
                          @endphp
                          @if(in_array(auth()->user()->program_id,$aProgramAvailability))
                            @if($supervisor->id == $item->assigned_to && count($aInprogressItems) < config('items.no_of_thesis.totalallotment'))
                              <option value="{{ $supervisor->id }}" selected>{{ $supervisor->name }}</option>
                            @elseif(count($aPreferedSuperVisor) > 0 )
                              @if($aPreferedSuperVisor[0]->user_id == $supervisor->id && count($aInprogressItems) < config('items.no_of_thesis.totalallotment'))
                               <option value="{{ $supervisor->id }}" selected>{{ $supervisor->name }}</option>
                              @endif
                            @else
                ...
                <div class="row">
                  <label class="col-sm-3 col-form-label form_chg">{{ __('Aim') }}</label>
                  <div class="col-sm-7 pt-3">
                ...
                <div class="row">
                  <label class="col-sm-3 col-form-label form_chg">{{ __('Objectives') }}</label>
                  <div class="col-sm-7 pt-3">
              ...
              <div class="card-footer ml-auto mr-auto">
        				@if((auth()->user()->role_id == 4 && $item->status == 1 && auth()->user()->id == $item->created_by ))
...
<script>
  ...
  function funValidateFormInput() {
   // alert(extraObj.fileCounter);
    if($('#name').val() == "") {
      swal("", "Please enter thesis title", "error").then(function() {
        $('#name').focus();
         return false;      
      }); 
