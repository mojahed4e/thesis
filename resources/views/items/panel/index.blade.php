                           	...
                           	@php
	                          	$term1presentaion =Illuminate\Support\Facades\DB::table('terms_progress_checklist')->select('terms_progress_checklist.*')
            									...
            									$aRubricTerm1Info = Illuminate\Support\Facades\DB::table('thesis_rubric_details')->select('thesis_rubric_details.*')
            											...
            											'thesis_rubric_details.rubric_type' => 2,'status' => 1])->get(); 
		            							...
		            							$aRubricTerm2Info = Illuminate\Support\Facades\DB::table('thesis_rubric_details')->select('thesis_rubric_details.*')
		            											...
		            											'thesis_rubric_details.rubric_type' => 3,'status' => 1])->get();  		            							        									
	                      		@endphp
	                          <td>
																		...
																		@if($request->id == $item->request_detail_id)
																			<button type="button" class="btn btn-success btn-link text-capitalize" data-original-title="" style="cursor:default" title="" onclick="#">
																				...
																				@elseif(count($aTermChecklist) > 0)
																					...
																					@endif
																				@else
																					@if(count($term1presentaion) > 0 && count($termIIpresentaion) == 0)
																						@if($term1presentaion[0]->approval_status == 3 && count($aRubricTerm1Info) == 0)
																							{{ __('Awaiting for') }} <br />{{ __('Grading') }}
																						@elseif(($term1presentaion[0]->approval_status != 1 || $term1presentaion[0]->approval_status == 3) && count($aRubricTerm1Info) > 0)
																							{{ __('Term - I') }}<br /> {{ __('In Progress') }}
																						@elseif($term1presentaion[0]->approval_status == 0 || $term1presentaion[0]->approval_status == 2)
																							{{ __('Awaiting for') }} <br />{{ __('Manager') }}<br />{{ __(' Approval') }}						
																						@endif
																					@elseif(count($termIIpresentaion) > 0)
																						@if($termIIpresentaion[0]->approval_status == 3)
																							{{ __('Awaiting for') }} <br />{{ __('Grading') }}
																						@elseif(($termIIpresentaion[0]->approval_status != 1 || $termIIpresentaion[0]->approval_status == 3) && count($aRubricTerm2Info) > 0)
																							{{ __('Term - II') }}<br /> {{ __('In Progress') }}
																						@elseif($termIIpresentaion[0]->approval_status == 0 || $termIIpresentaion[0]->approval_status == 2)
																							{{ __('Awaiting for') }} <br />{{ __('Manager') }}<br />{{ __(' Approval') }}	
																						@else
																							{{ __('Thesis') }} <br />{{ __('Successfully') }}<br />{{ __(' Completed') }}					
																						@endif
																					@else
																						{{ __('Awaiting for') }} <br />{{ __('Manager') }}<br />{{ __(' Approval') }}	
																					@endif																		
	                            ...
	                            @php	                          
		                      		...
		                      		@endphp	
															@if(count($term1Chapter2) > 0)																
																<div class="col-7 cht_text pt-1">																					
																	<span class="fileinput-new"><a href="{{ route('download.viewfile', [$term1Chapter2[0]->id,'type=term']) }}" target="_blank" style="color:rgb(61, 68, 101) !important;text-decoration: underline;">{{ __('View File') }}</a></span>
																</div>
															@else
																<span class="view_word" data-original-title="" style="font-size: 14px; color:green !important;"  title="" onclick="#">
																		Awaiting for <br />file Approval
	                          ...
	                          <td> 
													  @if(count($term1presentaion) > 0)
																@if(count($aRubricTerm1Info) > 0)
																	<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="javascript:void(0)" onclick="funViewRubric(1,2,{{$item->id}})" data-original-title="" title="">View Rubric</a>
																@else
																	@if($term1presentaion[0]->approval_status == 3)
																		<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="javascript:void(0)" onclick="funPrepareRubric(1,2,{{$item->id}})" data-original-title="" title="">Prepare Rubric</a>
																	@else
													  ...
													  <td>													   	
															@if(count($termIIpresentaion) > 0)																				
																	<span class="fileinput-new"><a href="{{ route('download.viewfile', [$termIIpresentaion[0]->id,'type=term']) }}" target="_blank" style="color:rgb(61, 68, 101) !important;text-decoration: underline;">{{ __('View File') }}</a></span>
															@else
																<span class="view_word" data-original-title="" style="font-size: 14px; color:green !important;"  title="" onclick="#">
																	Awaiting for <br />file Approval
																</span>
		                      		...
		                      		@endphp	                      																
													  ...
													  <td>
													  	@if(count($termIIpresentaion) > 0)
																@if(count($aRubricTerm2Info) > 0)
																	<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="javascript:void(0)" onclick="funViewRubric(2,3,{{$item->id}})" data-original-title="" title="">View Rubric</a>
																@else
																	@if($termIIpresentaion[0]->approval_status == 3)
																		<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="javascript:void(0)" onclick="funPrepareRubric(2,3,{{$item->id}})" data-original-title="" title="">Prepare Rubric</a>
																	@else
																		<span  data-original-title="" style="cursor:default;color:red;"  title="" onclick="#">
																			Pending
																		</span>
...
@push('js')  
  <script>
    ...
    function funViewRubric(pmTerm,pmType,pmItemID) {    	
    	$("#rubricterm").val(pmTerm);
    	$("#rubrictype").val(pmType);
    	$("#item_id").val(pmItemID);
    ...
    function funPrepareRubric(pmTerm,pmType,pmItemID) {    	
    	$("#rubricterm").val(pmTerm);
    	$("#rubrictype").val(pmType);
    	$("#item_id").val(pmItemID);
