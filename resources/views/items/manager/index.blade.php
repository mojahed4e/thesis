                          ...
                          <td>
                            @php
                          	...
                          	@if($item->status == 1 && $item->requested_by > 0 && $item->approval_status != 1)
                          		@if($item->request_approval_flag == 1 && empty($prefsupervisor) && $item->approval_status == 0)
                          		...
                          		@else
															...
															@elseif ($item->requested_by == 0 && $item->status == 1 && $item->request_approval_flag == 0)
																...
																@elseif ($item->requested_by > 0 && $item->status == 1 && $item->request_approval_flag == 1 && $item->approval_status == 0)
																<button type="button" class="btn btn-success btn-link text-capitalize" data-original-title="" style="cursor:default" title="" onclick="#">
																	<div class="ripple-container" style="overflow:visible;min-width:80px;">{{ __('Awaiting for') }} <br />{{ __('Supervisor') }}<br />{{ __(' Assignment') }}</div> 
																</button>
															@else
																			...
																			@can('update', $item)																					
																				@foreach ($requestinfo as $request)	
																					@if($request->item_id == $item->id)
																						...
																						@elseif($request->id == $item->request_detail_id &&  $request->supervisor_acceptence_status == 0 && auth()->user()->role_id == 2 && $item->assigned_to == auth()->user()->id)
																							<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="{{ route('mythesis.detail', [$item->id]) }}" data-original-title="" title="">							
																							Accept Assignment
																							</a>
																						@elseif(auth()->user()->role_id == 2)
																							@if($item->request_approval_flag == 1 && !empty($prefsupervisor))
																							...
																							@elseif($item->request_approval_flag == 1 && empty($prefsupervisor) && $item->approval_status == 0)
																								<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="{{ route('item.approve', [$item->id]) }}" data-original-title="" title="">								
																									Assign & Approve
																								</a>
																							@else
