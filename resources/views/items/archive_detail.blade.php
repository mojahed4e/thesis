@if(auth()->user()->role_id == 2 && ($item->assigned_to != auth()->user()->id || (($item->assigned_to == auth()->user()->id && $requestdetails[0]->completion_by_supervisor == 2 && $requestdetails[0]->completion_by_manager == 1) || (($requestdetails[0]->completion_by_supervisor == 3 && $requestdetails[0]->completion_by_manager == 2)))))
	...
	$pagetitle = 'Archive Thesis Details';
	@endphp
@elseif(auth()->user()->role_id == 3 || (auth()->user()->role_id == 2 && $item->assigned_to == auth()->user()->id ))
	...
	$pagetitle = 'Assigned Thesis Details';
	@endphp
@else
	@php
								...
								<div class="card-body ">
									...
									<div class="tab-content tab-space">
										<div class="tab-pane active" id="link1">
											...
											<dir class="row">
												<div class="col-md-6">
													<table class="table">    
														...
														<tr>
															<td class="cbold_text">{{ __('Category') }}</td>
															<td class="sid_text">
												...
												<div class="col-md-6">
														<table class="table">
															...
															<tr>
																<td class="cbold_text">{{ __('Group Members') }}</td>
																<td class="sid_text">
																	@php
																	$i = 0
																	@endphp	
											...
											<div class="row pl-5">
												<h6 class="cbold_text pb-2 pl-3 text-capitalize">{{ __('Description') }}</h6>
												<div class="sid_text pt-3 col-md-10" style=" text-align:justify;">{!! $item->description !!}</div>
											</div>
											@if(auth()->user()->role_id == 2 && $item->approval_status == 1 && request()->get('action') != "ac")
												...
												<div id="divShowTermDates" @if($requestdetails[0]->manager_approval_status == 0) style="display:none" @endif >
													<div class="row">
													  ...
													  <div class="col-sm-3">
														<div class="form-group view_word {{ $errors->has('term1date') ? ' has-danger' : '' }}">
															<input type="text"  name="term1date" id="term1date" placeholder="{{ __('Select date') }}" class="form-control datetimepicker" value="{{ old('term1date', ($requestdetails[0]->termI_completion_date ? \Carbon\Carbon::parse($requestdetails[0]->termI_completion_date)->format('d-m-Y') : "")) }}"/>						
													...
													<div class="row">
													  ...
													  <div class="col-sm-3">
														<div class="form-group view_word {{ $errors->has('term2date') ? ' has-danger' : '' }}">
															<input type="text"  name="term2date" id="term2date" placeholder="{{ __('Select date') }}" class="form-control datetimepicker" value="{{ old('term2date', ($requestdetails[0]->termII_completion_date ? \Carbon\Carbon::parse($requestdetails[0]->termII_completion_date)->format('d-m-Y') : "")) }}"/>						
															@include('alerts.feedback', ['field' => 'term2date'])
														</div>
													  </div>
													</div>
													<div class="row">
													  <label class="col-sm-3 col-form-label cht_text">{{ __('Term - III Completion Date') }}</label>
													  <div class="col-sm-3">
														<div class="form-group view_word {{ $errors->has('term3date') ? ' has-danger' : '' }}">
															<input type="text"  name="term3date" id="term3date" placeholder="{{ __('Select date') }}" class="form-control datetimepicker" value="{{ old('term3date', ($requestdetails[0]->termIII_completion_date ? \Carbon\Carbon::parse($requestdetails[0]->termIII_completion_date)->format('d-m-Y') : "")) }}"/>						
															@include('alerts.feedback', ['field' => 'term3date'])
														</div>
													  </div>
													</div>
												</div>
												<div class="row" style="text-align:center;">
													...
													<div class="col-sm-7">
														...
														@if(auth()->user()->manager_flag != 2 && $requestdetails[0]->progress_completion != 3)
															<button type="button" onclick="funUpdateThesisDetails()" class="btn bt_styl btn_txtbold">{{ __('Save') }}</button>
														@endif														
													</div>
												</div>
											@endif
											@if(auth()->user()->role_id == 3 || (auth()->user()->role_id == 2 && $item->assigned_to	== auth()->user()->id))	
												...
												<div class="row">
													<label class="col-sm-2 col-form-label cht_text"><span class="mark">*</span>{{ __('Accept Status') }}</label>
													<div class="col-sm-7">														
												...
												<div id="divApproveMessage" style="display:none">
													<div class="row">
														<label class="col-sm-2 col-form-label cht_text"><span class="mark">*</span>{{ __('Description') }}</label>
														<div class="col-sm-7">
														...
														<div class="card-body">
															...
															@if(count($trackinginfo['progress']))
																@for ($loop = 0; $loop<count($trackinginfo['progress']); $loop++) 
																	@if(auth()->user()->role_id != 4 && !empty($trackinginfo['progress'][$loop]->description))																
																		...
																		@endif
																	@else
																		@if($trackinginfo['progress'][$loop]->message_type == 1 && !empty($trackinginfo['progress'][$loop]->description))
																			@if(auth()->user()->id == $trackinginfo['progress'][$loop]->user_id)
																			<li class="timeline-inverted">
																				<div class="timeline-badge">
																					<img src="{{ config('items.image_auth.path') }}{{ auth()->user()->profilePicture() }}" style="width:2.8rem; height:2.8rem;border-radius:30px; " />
										...
										<div class="tab-pane" id="link4">
											@include('items.archive_termprogress')
										</div>
										@endif
									</div>					  
								</div>
