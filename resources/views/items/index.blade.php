        ...
        <div class="col-md-12">
            <div class="card">
              ...
              <div class="card-body">	
                		...
                		<div class="row">
                					<div class="col-1 form_chg text-right pt-2">														
												...
												<!--	<div class="col-2 form_chg text-left">														
														<span>
															<select class="form-control selectpicker" data-style="btn btn-link" id="selAcademicyear" onchange="funFilderTableData(this.value,this.id)">
													      <option value="">Show All</option>
													      @if(count($academic_years) > 0)
														      @foreach ($academic_years as $academic_year)
					                         	<option value="{{ $academic_year->id }}" {{ $academic_year->id == old('selAcademicyear') ? 'selected' : '' }}>{{ $academic_year->academic_year }}</option>
					                        @endforeach
					                      @endif
											    		</select>
											    	</span>														
                ...
                @endcan
                @if(auth()->user()->role_id == 4)                	
                	@if(count($studentitem) == 0 && $group_member == 0)
	                  <div class="row">
                                ...
                                <form action="{{ route('item.destroy', $item) }}" method="post">
                                    ...
                                    @method('delete')                                    
																			...
																			@can('update', $item)		
																				...
																				@foreach ($requestinfo as $request)	
																					...
																					@if($request->item_id == $item->id)
																						...
																						@endif
																					@elseif($request->supervisor_acceptence_status == 1 && auth()->user()->role_id == 2)															
																							<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="{{ route('mythesis.detail', [$item->id]) }}" data-original-title="" title="">							
																							View Progress
																							</a>
																					@endif
																		...
																		@elseif($item->requested_by)
																			...
																			<a rel="tooltip" class="btn bt_styl text-capitalize" style="padding: 0.40625rem 0.60rem;line-height: 0.70;" href="{{ route('item.approve', [$item->id]) }}" data-original-title="" title="">Approve</a>
																			@else
                                   ...
                                   @if (($item->requested_by == 0 && $item->assigned_to == 0 && $item->approval_status == 0) && auth()->user()->can('delete', $item) && auth()->user()->manager_flag != 2)
                                      <button type="button" class="btn btn-link" data-original-title="" title="" onclick="confirm('{{ __("Are you sure you want to delete this thesis?") }}') ? this.parentElement.submit() : ''"><i class="material-icons" style="font-size: 2rem;">cancel</i>
                                      <div class="ripple-container"></div>
                                      </button>
                                    @endcan
                                </form>
