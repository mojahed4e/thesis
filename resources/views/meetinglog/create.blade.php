...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" id="frmMeetingLogs" name="frmMeetingLogs" action="{{ route('meetinglogs.store') }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')
            ...
            <div class="card ">
              <div class="card-header card-header-rose card-header-icon">
                ...
                <h4 class="card-title">{{ __('Add Meeting Log') }}</h4>
              </div>
              <div class="card-body ">
                <div class="row">
                  <div class="col-md-12 text-right">
                      <a href="{{ route('meetinglogs.index','id='.request()->id) }}" class="btn btn-sm btn-rose">{{ __('Back to list') }}</a>
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                       <input type="text" name="meeetingdate" id="meeetingdate"
						  placeholder="{{ __('Select date') }}" class="form-control datetimepicker" value="{{ old('meeetingdate', now()->format('d-m-Y')) }}"/>
				...
				<div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('lastmilestoneachieved') ? ' has-danger' : '' }}">
                      <textarea cols="30" rows="15" class="form-control{{ $errors->has('lastmilestoneachieved') ? ' is-invalid' : '' }}" name="lastmilestoneachieved" id="lastmilestoneachieved" type="text" placeholder="{{ __('Work Undertaken Since Last Meeting/Last Milestone Achieved') }}" required="true" aria-required="true">{{ old('description') }}</textarea>
				...
				<div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('discussthismeeting') ? ' has-danger' : '' }}">
                      <textarea cols="30" rows="15" class="form-control{{ $errors->has('discussthismeeting') ? ' is-invalid' : '' }}" name="discussthismeeting" id="discussthismeeting" type="text" placeholder="{{ __('Issues/Progress You Would Like To Discuss In This Meeting') }}" required="true" aria-required="true">{{ old('description') }}</textarea>
				...
				<div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('nextmeetingagenda') ? ' has-danger' : '' }}">
                      <textarea cols="30" rows="15" class="form-control{{ $errors->has('nextmeetingagenda') ? ' is-invalid' : '' }}" name="nextmeetingagenda" id="nextmeetingagenda" type="text" placeholder="{{ __('Student Should Undertake Work Between Now And Next Meeting(Next Meeting Agenda Points') }}" required="true" aria-required="true">{{ old('description') }}</textarea>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                      <input type="text" name="nextmeetdate" id="nextmeetdate"
						  placeholder="{{ __('Select date') }}" class="form-control datetimepicker" value="{{ old('nextmeetdate', now()->format('d-m-Y')) }}"/>
              ...
              <div class="card-footer ml-auto mr-auto">				
				...
				<button type="button" id="postcomment" onclick="funAddMeetingLogInfo()" class="btn btn-success">{{ __('Add Meeting Log') }}</button>
				<a href="{{ route('meetinglogs.index','id='.request()->id) }}" class="btn btn-rose">{{ __('Cancel') }}</a>
              </div>
            </div>
          </form>
        </div>
